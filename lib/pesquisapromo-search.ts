...
export function resolveCityDestination(query: string): CityDestination | null {
  ...
  if (normalizedQuery.includes("fone")) {
    return cityDestinations.find((item) => item.slug === "lojinha-alfa") ?? null;
  }
  if (normalizedQuery.includes("teste") || normalizedQuery.includes("sandbox") || normalizedQuery.includes("laboratorio")) {
    return cityDestinations.find((item) => item.slug === "lojinha-alfa") ?? null;
  }
...
export function searchCityNavigation(query: string): SearchIntentResult {
  ...
  if (bestPath?.slug === "lojinha-alfa" && relatedOffers.length === 0) {
    relatedOffers.push(...getOffersByShopSlug("lojinha-alfa"));
  }
